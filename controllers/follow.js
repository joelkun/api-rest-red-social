...
const following = async (req, res) => {
  try {
    ...
    return res.status(500).send({
      status: "Error",
      message: "Error en consulta de datos de usuario",
    });
  }
};
...
const followers = async (req, res) => {
  try {
    ...
    //Usuarios por pagina quiero mostrar
    const itemsPerPage = 5;
    const countFollow = await Follow.countDocuments({ followed: userID });
    const findFollow = await Follow.find({ followed: userID })
      .populate("user followed", "-password -role -__v -email")
      .paginate(page, itemsPerPage);
    ...
    return res.status(500).send({
      status: "Error",
      message: "Error en consulta de datos de usuario " + error,
    });
  }
};
